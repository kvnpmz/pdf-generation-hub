...
public class LayoutStrategy : IPdfGeneratorStrategy
{
    ...
    public void Generate(
      ...
      HashSet<int>? overrideColumns = null)
    {
        GenerateCore(fileName, fileTitle, pageSize, columns, theme, overrideColumns);
    }

    private void GenerateCore(string fileName, string fileTitle, PageSize pageSize, List<DataColumn> columns, ReportTheme theme, HashSet<int>? overrideColumns)
    {
        ...
        var metrics = new LayoutMetrics(
          H1Height: ScaleFromLetter(theme.BaseH1Height, height),
          H2Height: ScaleFromLetter(theme.BaseH2Height, height),
          ItemHeight: ScaleFromLetter(theme.BaseItemHeight, height),
          H2TotalHeight: ScaleFromLetter(theme.BaseH2Height, height) + theme.H2PaddingBottom
        );
    ...
    private void BuildJustifiedColumn(IContainer container, float totalHeight, int colIndex, DataColumn data, ReportTheme theme, bool isA4, HashSet<int> overrideColumns, LayoutMetrics metrics, SectionCounter counter) 
    {
        ...
        int currentMaxItemChars = theme.MaxItemChars;
        ...
        float usedSpace = (actualTitleLineCount * metrics.H2Height) + (totalSections * theme.H2PaddingBottom) + (actualLineCount * currentItemHeight);
