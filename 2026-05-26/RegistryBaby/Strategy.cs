...
{
    private const float VerticalMargin = 36f;
    private const float HorizontalMargin = 50.4f;
    private const float ColumnSpacing = 23f;
    private const string HeaderColor = "#3b6d6d";
    private const string SubheaderColor = "#0000ff";
    ...
    public void Generate(string fileName, string fileHeader, PageSize pageSize, List<DataColumn> columns, ReportTheme theme)
    {
        ...
        float availableHeight = dimensions.Height - (VerticalMargin * 2) - totalHeaderHeight - totalSubheaderHeight;
                        ...
                        foreach (var lineText in headerLines)
                        {
                            headerCol.Item().Height(layoutMetrics.HeaderHeight).Layers(layers =>
                            {
                                layers.PrimaryLayer().AlignTop().AlignCenter().Text(lineText.ToUpper())
                                    .Style(TextStyle.Default.FontSize(theme.HeaderFontSize).Weight(FontWeight.ExtraBold).LetterSpacing(0.046f).FontColor(HeaderColor));
                            });
                        }
                    });
                    ...
                    if (subheaderLines.Count > 0)
                    {
                        rootCol.Item().Column(subheaderCol =>
                        {
                            foreach (var lineText in subheaderLines)
                            {
                                ...
                                {
                                    layers.PrimaryLayer().AlignTop().AlignCenter().Text(lineText.ToUpper())
                                        .Style(TextStyle.Default.FontSize(theme.SubheaderFontSize).Weight(FontWeight.ExtraBold).LetterSpacing(0.046f).FontColor(SubheaderColor));
                                });
                            }
                        });
                    }
...
public static class ColumnLayoutCalculator
{
    ...
    {
        int maxTitleChars = colIndex == 4 ? 15 : theme.MaxTitleChars;
        int maxSubtitleChars = theme.MaxSubtitleChars;
        int maxItemChars = theme.MaxItemChars;
