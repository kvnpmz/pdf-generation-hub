...
public class LayoutStrategy : IPdfGeneratorStrategy {
    ...
    const int MaxTitleChars = 16;
    const int MaxItemChars = 35;
            ...
            container.Page(page => {
                ...
                page.Content().Column(rootCol => {
                    rootCol.Item().Height(H1Height).Layers(layers => {
                        layers.PrimaryLayer().AlignTop().AlignCenter().Text("Anti Inflamatory Food List".ToUpper())
                    ...
                    foreach (var item in section.Items) {
                        ...
                        for (int j = 0; j < lines.Count; j++) {
                            inner.Item().Height(ItemHeight).Layers(l => {
                                if (j == 0) l.Layer().AlignLeft().PaddingTop(BulletPaddingTop).Width(4f).Height(4f).Background(theme.PrimaryColor).CornerRadius(2.25f);
