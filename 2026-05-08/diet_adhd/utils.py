...
def calculate_dynamic_margins(html_content, page_height, page_width, scale):
    ...
    """
    container_height = page_height - MARGIN - H1_HEIGHT - H2_HEIGHT - H1_MARGIN - 2 * (SECTION_BORDER + SECTION_PADDING)
    soup = BeautifulSoup(html_content, 'html.parser')
    columns = soup.find_all(class_='section')
    
    margins = []
    
    for col in columns:
        items = col.find_all(class_='item')
        num_items = len(items)
        
        item_count = len(col.find_all(class_='item'))

        used_space = (item_count * ITEM_HEIGHT * scale)
        leftover_space = container_height - used_space
        print(leftover_space)

        margin_per = leftover_space / (num_items - 1)
